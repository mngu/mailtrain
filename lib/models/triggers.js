    ...
    db.getConnection((err, connection) => {
        ...
        connection.query(query, (err, rows) => {
            ...
            let triggers = (rows || []).map(tools.convertKeys).map(row => {
                ...
                row.formatted = util.format('%s seconds after %s', row.seconds, row.rule === 'subscription' ? row.columnName : (util.format('%s <a href="/campaigns/view/%s">%s</a>', defaultEventMap[row.column], row.sourceCampaign, row.sourceCampaignName)));
...
module.exports.create = (trigger, callback) => {
    ...
    let seconds = Number(trigger.seconds) || 0;
    ...
    if (seconds < 0) {
        return callback(new Error(_('Seconds in the past are not allowed')));
    }
...
module.exports.update = (id, trigger, callback) => {
    ...
    let seconds = Number(trigger.seconds);
    let rule = (trigger.rule || '').toString().toLowerCase().trim();
    let destCampaign = Number(trigger.destCampaign) || 0;
    let sourceCampaign = null;
    let column;
